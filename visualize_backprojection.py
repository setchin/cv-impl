...
def detect_top_features(integral: np.ndarray, max_corners: int = 10) -> np.ndarray:
    ...
    # (N, 1, 2) -> (N, 2), sorted by Shi-Tomasi response order from OpenCV
...
def main() -> None:
    ...
    if not images:
        raise RuntimeError("No input images found in input_Image.")
    if len(images) != len(x_positions):
        raise RuntimeError(
            f"Expected {len(x_positions)} images, got {len(images)}. "
            "Please update x_positions/y_positions or input data."
        )

    dem_flat = DEM()
    dem_flat.load_obj("zero_plane.obj")
    ...
    output_dir = Path("results") / "backprojection"
